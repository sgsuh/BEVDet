...
template<typename T>
struct Serializer<T, typename std::enable_if<std::is_arithmetic<T>::value || std::is_enum<T>::value || std::is_pod<T>::value>::type> {
    static size_t serializedSize(T const& value) {
        return sizeof(T);
    }
...
struct Serializer<const char*> {
    static size_t serializedSize(const char* value) {
...
template<typename T>
struct Serializer<std::vector<T>, typename std::enable_if<std::is_arithmetic<T>::value || std::is_enum<T>::value || std::is_pod<T>::value>::type> {
