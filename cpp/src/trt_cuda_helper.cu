...
__global__ void copyPermuteKernel(scalar_t* __restrict__ dst,
...
void memcpyPermute(scalar_t* dst, const scalar_t* src, int* src_size, int* permute, int src_dim, cudaStream_t stream) {
    ...
    copyPermuteKernel<scalar_t><<<getBlocks(copy_size), THREADS_PER_BLOCK, 0, stream>>>(dst, src, copy_size, ts_src_stride, ts_dst_stride, ts_permute);
}
